...
import { validateAmount, validateCurrency, sanitizeInput } from "@/lib/validation";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!recipient || !amount || !currency) {
      return NextResponse.json(
        { success: false, error: "Recipient, amount, and currency are required" },
        { status: 400 }
      );
    }
    ...
    if (typeof amount !== "number" || !validateAmount(amount)) {
      return NextResponse.json(
        { success: false, error: "Amount must be a positive number within allowed limits" },
        { status: 422 }
      );
    }
    ...
    // Send OTP to sender
    const emailResult = await sendGiftConfirmationOTP(userEmail, otp, recipientUser.name || undefined);
